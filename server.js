...
const statusPath = path.resolve(stateDirectory, "mail-status.json");
let isSending = false;
...
function getStatusKey(listId, email) {
  return `${listId}::${email.toLowerCase()}`;
}

function writeStatusStore(store) {
  fs.writeFileSync(statusPath, JSON.stringify(store, null, 2));
}

function updateRecipientStatus(listId, email, patch) {
  const store = readStatusStore();
  const key = getStatusKey(listId, email);
  store[key] = {
    ...store[key],
    ...patch,
...
function resolveCsvFile(listId) {
  ...
  if (files.length === 0) {
    throw new Error("No certification CSV files were found in server/data.");
  }
...
function decorateRecipientsWithStatus(listId, recipients) {
  const store = readStatusStore();
  return recipients.map((recipient) => ({
    ...recipient,
    delivery: store[getStatusKey(listId, recipient.email)] ?? {
      status: "pending",
      label: "No Done",
      reason: "",
      updatedAt: null
...
app.get("/api/recipients", (_req, res) => {
  try {
    ...
    res.json({
      list,
      recipients: decorateRecipientsWithStatus(list.id, recipients),
      skipped
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
app.post("/api/send", async (_req, res) => {
  ...
  try {
    ...
    const recipientsWithStatus = decorateRecipientsWithStatus(list.id, recipients);
    const requestedBatchSize = Number.parseInt(String(_req.body?.batchSize ?? ""), 10);
    const pendingRecipients = recipientsWithStatus.filter((recipient) => recipient.delivery.status !== "sent");
    const batchSize =
      Number.isFinite(requestedBatchSize) && requestedBatchSize > 0
        ? Math.min(requestedBatchSize, pendingRecipients.length)
        : pendingRecipients.length;
    const recipientsToSend = pendingRecipients.slice(0, batchSize);
    ...
    for (const recipient of recipientsToSend) {
      try {
        ...
        const delivery = updateRecipientStatus(list.id, recipient.email, {
          status: "sent",
          label: "Done",
          reason: ""
        });
        ...
        const delivery = updateRecipientStatus(list.id, recipient.email, classifiedError);
        results.push({
          email: recipient.email,
          delivery
        });
        ...
        if (classifiedError.status === "blocked") {
          return res.status(429).json({
            ...
            remainingCount: Math.max(pendingRecipients.length - (sent.length + failed.length), 0),
            list,
            sent,
            failed,
            results,
    ...
    return res.json({
      message: `Processed ${recipientsToSend.length} pending email(s). ${sent.length} done, ${failed.length} no done, ${skipped.length} skipped from CSV validation.`,
      processedCount: recipientsToSend.length,
      remainingCount: Math.max(pendingRecipients.length - recipientsToSend.length, 0),
      list,
      sent,
      failed,
      results,
